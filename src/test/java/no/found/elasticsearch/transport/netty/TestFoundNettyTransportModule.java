...
public class TestFoundNettyTransportModule {
    ...
    public void testBackwardsCompatibilityWithOnlyClientUsingModule() throws Exception {
        ...
        try {
            ...
            NodesInfoResponse nodesInfo = node1.client().admin().cluster().prepareNodesInfo().setTransport(true).execute().get();
            ...
            assertEquals(ClusterHealthStatus.GREEN, transportClient.admin().cluster().prepareHealth().execute().get().getStatus());
        } finally {
            try {
